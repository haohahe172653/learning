...
public class CourseRegisterServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        Account d = (Account) session.getAttribute("user");
        int flag = 0;
        //check join course if need to add or not
        String[] joinCourseList = cdao.AccountJoinCourseAll(d.getId()).split("\\.\\.");
        for (int i = 0; i < joinCourseList.length; i++) {
            if(joinCourseList[i].equals(courseId)){
                flag = 1;
                break;
            }
        }
        //add to the list
        if (flag == 0){
            cdao.CourseRegister(d.getId(), courseId);
        }
